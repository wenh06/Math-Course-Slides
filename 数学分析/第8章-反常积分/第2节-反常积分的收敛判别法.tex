...
\begin{cor}
设 $f(x)$ 在 $[1, \omega)$ 上非负不增, 且局部可积, 那么级数 $\displaystyle \sum_{n=1}^{\infty} f(n)$ 与反常积分 $\displaystyle \int_1^{\omega} f(x) ~ \mathrm{d} x$ 同敛散.
\end{cor}

\pause

\startproof[bg=yellow, fg=red]
由于 $f(x)$ 在 $[1, \omega)$ 上非负不增, 则对于任意的 $n \in \mathbb{N}^+$ 和 $x \in [n, n+1),$ 有
\[
...
\]
这就说明了 $\displaystyle \sum_{n=1}^{\infty} f(n)$ 与 $\displaystyle \int_1^{\omega} f(x) ~ \mathrm{d} x$ 同敛散.
\qed
...
\begin{cor}
取 $f(x) = \frac{1}{x^s}$, 则 $\displaystyle \zeta(s) := \sum_{n=1}^{\infty} \frac{1}{n^s}$ 与 $\displaystyle \int_1^{\omega} \frac{1}{x^s} ~ \mathrm{d} x$ 同敛散. 因此 $\zeta(s)$ 在 $s > 1$ 时收敛, 在 $s \leqslant 1$ 时发散.
\end{cor}
