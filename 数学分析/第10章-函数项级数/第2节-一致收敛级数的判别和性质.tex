...
{\color{red}\Circled{3}} 取 $N(\varepsilon)$ 使得 $\forall ~ n \geqslant N(\varepsilon),$ $\forall ~ x \in [a, b],$ $|S_n(x) - S(x)| < \varepsilon/3.$

\pause
\vspace{0.5em}

{\color{red}\Circled{1}} 特别地, 有 $|S_n(x_0) - S(x_0)| < \varepsilon/3.$
...
\startproof[bg=yellow, fg=red]
由一致收敛函数列极限函数的连续性定理, 可以得出 $S(x)$ 在 $[a, b]$ 上连续, 从而 $S(x) \in \mathfrak{R}[a, b].$
...
这是因为条件 (3) $S_n'(x) \stackrel{[a, b]}{\rightrightarrows} \sigma(x)$ $\implies$ $\forall ~ \varepsilon > 0,$ $\exists ~ N_2(\varepsilon) \in \mathbb{N}$ 使得 $\forall ~ n \geqslant N_1(\varepsilon),$ $\forall ~ x \in [a, b],$ $|S_n'(x) - \sigma(x)| < \varepsilon.$ 那么
\[
\left| S_n(x) - S(x) - (S_n(a) - S(a)) \right| = \left| \int_a^x (S_n'(t) - \sigma(t)) ~ \mathrm{d}t \right| \leqslant \int_a^x |S_n'(t) - \sigma(t)| ~ \mathrm{d}t \leqslant \varepsilon (b - a).
\]

\pause
\vspace{0.5em}

由于 $S_n(a) \to S(a),$ $\exists ~ N_2(\varepsilon) \in \mathbb{N}$ 使得 $\forall ~ n \geqslant N_2(\varepsilon),$ $|S_n(a) - S(a)| < \varepsilon.$ 取 $N(\varepsilon) = \max \{ N_1(\varepsilon), N_2(\varepsilon) \},$ 当 $n \geqslant N(\varepsilon)$ 时, 对任意 $x \in [a, b],$ 都有
\[
\left| S_n(x) - S(x) \right| \leqslant \left| S_n(x) - S(x) - (S_n(a) - S(a)) \right| + |S_n(a) - S(a)| < 2\varepsilon.
\]
从而 $\{S_n(x)\}$ 在 $[a, b]$ 上一致收敛到 $S(x).$
\end{remark}
...
\begin{eg}
证明当 $x \in (-1, 1)$ 时, 成立 $\displaystyle \sum_{n=1}^\infty n x^{n} = \frac{x}{(1 - x)^2}.$
\end{eg}
...
\end{itemize}
\pause
综合起来, 可以对函数项级数 $\displaystyle \sum_{n=1}^\infty x^n$ 使用逐项求导定理, 得到 $\displaystyle \sum_{n=1}^\infty n x^{n-1} = \frac{1}{(1 - x)^2},$ 从而 $\displaystyle \sum_{n=1}^\infty n x^{n} = \frac{x}{(1 - x)^2}.$
\qed
...
{\color{red}\ding{43}} 注记5: 本节介绍的一致收敛函数项级数和函数 (函数列极限函数) 分析性质, 大部分是以收敛域 (或定义域) 是 \alert{有限区间} 为前提的. 对于无穷区间, 例如 $[a, +\infty)$, 或者 $(-\infty, +\infty)$ 上的一致收敛函数项级数和函数 (函数列极限函数), 其分析性质的结论 \alert{可能不成立}.

\pause
\vspace{0.5em}

例如 $\displaystyle \sum_{n=1}^\infty \frac{x}{n^2 x^2 + 1}$ 在 $(-\infty, +\infty)$ 上一致收敛到 $0,$ 但是 $\displaystyle \lim_{n\to\infty} \int_{-\infty}^{+\infty} \frac{x}{n^2 x^2 + 1} ~ \mathrm{d}x = \pi \neq 0 = \int_{-\infty}^{+\infty} 0 ~ \mathrm{d}x.$

\pause
\vspace{0.5em}

其原因在于, 在无穷区间上, 函数项级数和函数 (函数列极限函数) 的分析性质的结论, 可能涉及不止 2 个极限的交换, 上例还涉及第 3 个极限过程 (反常积分), 从而导致结论不成立. 这也是为什么我们在介绍一致收敛函数项级数和函数 (函数列极限函数) 的分析性质时, 都是以有限区间为前提的.
...
\[f(x) = \sum_{n=0}^\infty a^n \varphi(b^n x), ~~ 0 < a < 1, b \in 2\mathbb{N}, ab \geqslant 1.\]
...
\item 性质二: 若 $0 < a < 1,$ $b \in 2\mathbb{N}$ 为偶数, $ab \geqslant 1,$ 则 $\displaystyle f(x) = \sum_{n=0}^\infty a^n \varphi(b^n x)$ 在 $\mathbb{R}$ 上处处不可导.
\end{itemize}
...
\startproof[bg=yellow, fg=red](性质一的证明)
由于 $0 \leqslant \varphi(x) \leqslant 1/2,$
...
\startproof[bg=yellow, fg=red](引理证明)
记 $h_{1,n} = x_0 - u_n,$ $h_{2,n} = v_n - x_0,$ 那么 $h_{1,n}, h_{2,n} \geqslant 0$ 且 $h_{1,n} \to 0,$ $h_{2,n} \to 0.$ 由于 $g(x)$ 在点 $x_0$ 处可导, 所以在 $x_0$ 附近有
\begin{equation*}
...
\startproof[bg=yellow, fg=red](性质二的证明)
...
= \left. \pm a^m \dfrac{1}{2 b^{n-m}} \middle/ \left( \dfrac{1}{2 b^n}\right) \right.
= \pm (ab)^m.
\end{equation*}
