...
\textbf{解}: 由波义耳定律, 活塞位置 $x$ 处的压强为 $P(x) = \frac{k}{\pi R^2 x}$. 因此, 在位置 $x$ 上的力为
\[
...
\textbf{解}: 这是一个简单的可分离变量的微分方程. 将 $P$ 的项移到一边, $t$ 的项移到另一边, 得到 $\frac{\mathrm{d}P}{P} = r \mathrm{d}t$. 两边积分得到 $\ln P = rt + C$, 其中 $C$ 是积分常数. 由初始条件 $P(t_0) = P_0$, 可得 $C = \ln P_0 - rt_0$. 因此, 人口数量随时间的变化为
\[
P(t) = P_0 e^{r(t - t_0)}.
\]
