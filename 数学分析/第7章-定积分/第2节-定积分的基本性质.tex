...
\begin{frame}
\frametitle{复习：定积分的定义}
\begin{Def}[Riemann 积分]
设 $f$ 是定义在 $[a,b]$ 上的函数. 若存在实数 $I$, 使得对 $[a,b]$ 的任意带标志点的划分 $(P, \xi) = (\{x_i\}_{i=0}^n,\ \{\xi_i\}_{i=1}^n)$, 当划分细度 $\lambda(P) \to 0$ 时, 积分和 $S(f; P, \xi)$ 都收敛于 $I$:
\[ \lim_{\lambda(P) \to 0} \sum_{i=1}^n f(\xi_i) \Delta x_i = I \]
则称 $f$ 在 $[a,b]$ 上 \textbf{Riemann 可积}, $I$ 称为定积分, 记为 $\int_a^b f(x) ~ \mathrm{d}x$.
\end{Def}
\end{frame}
...
\begin{frame}
\frametitle{复习：可积性判别准则}
\begin{block}{必要条件}
若 $f \in \mathfrak{R}[a,b]$, 则 $f$ 在 $[a,b]$ 上\textbf{有界}.
\end{block}

\begin{block}{充分必要条件}
\begin{itemize}
\item \textbf{Cauchy 准则} (积分和的 Cauchy 收敛原理)
\item \textbf{Darboux 准则} (上积分与下积分相等: $\underline{\int_a^b} f ~ \mathrm{d}x = \overline{\int_a^b} f ~ \mathrm{d}x$)
\item \textbf{振幅准则} (振幅和 $\lim_{\lambda(P) \to 0} \sum \omega_i \Delta x_i = 0$)
\item \textbf{振幅准则简化版} ($\forall ~ \epsilon > 0, \exists ~ P$ 使得 $\sum \omega_i \Delta x_i < \epsilon$)
\item \textbf{Lebesgue 判别法} (有界且几乎处处连续)
\end{itemize}
\end{block}

\begin{block}{典型可积函数类}
连续函数 $C[a,b]$, 只有有限个间断点的有界函数, 单调函数.
\end{block}
\end{frame}
...
{
\larger[2]
在上一节中, 我们介绍了定积分的概念, 给出了定积分存在的充分必要条件---函数 Riemann 可积的判别法, 并得到了一个集合 (空间)
\[
\mathfrak{R}[a, b] \subsetneqq B[a, b].
\]
本节将研究这个集合中每个元素, 即每个 Riemann 可积函数所具有的一些基本性质, 进而可以看出这个集合的一些空间结构.
}
...
\begin{frame}
\frametitle{Riemann 积分关于被积函数的线性性}
...
对任意带标志点的划分 $(P, \xi) = (\{x_k\}_{k=0}^n,\ \{\xi_k\}_{k=1}^n)$, 有
\[
...
\startproof[bg=yellow, fg=red]
记所得函数为 $g,$ 并定义 $h(x) = g(x) - f(x).$ 则 $h(x)$ 在 $[a, b]$ 上仅在有限个点处不为零. 因此, $h$ 只在这些点处不连续, 其不连续点集为有限集, 故 $h \in \mathfrak{R}[a, b]$ 且容易算出 $\int_a^b h(x) ~ \mathrm{d} x = 0.$
\qed
...
\alert{结论是: 不行!} 改变常值函数 $f(x) = 0$ 在有理数集上的函数值, 可得到 Dirichlet 函数, 它在 $[a, b]$ 上不 Riemann 可积.

对于 Riemann 积分来说, 这个结论 (改变可列多个点处的函数值) 不成立, 但对于以后要学的\alert{Lebesgue 积分}来说, 这个结论是成立的.
...
\begin{frame}
\frametitle{Riemann 积分关于被积函数乘积的乘积可积性}
...
\startproof[bg=yellow, fg=red]
由于 $f, g$ 在 $[a, b]$ 上均有界, 故存在常数 $M > 0$, 使得对任意 $x \in [a, b]$, 有 $|f(x)| \leqslant M,$ $|g(x)| \leqslant M,$ 且 $|f(x) \cdot g(x)| \leqslant M.$ 由振幅判别法, 存在划分 $P$ 使得
\[
...
\]
由于 $f + g, f - g \in \mathfrak{R}[a, b],$ 且有界函数的平方仍然是有界函数, 故只需证明 Riemann 可积函数的平方是 Riemann 可积的即可, 这个直接可以通过振幅看出来:
\begin{align*}
...
\begin{frame}
\frametitle{Riemann 积分关于被积函数的保序性/单调性}
...
\startproof[bg=yellow, fg=red]
由线性性, 只要对 $f \geqslant 0$ 证明 $\int_a^b f(x) ~ \mathrm{d} x \geqslant 0$ 即可. 设 $(P, \xi) = (\{x_k\}_{k=0}^n,\ \{\xi_k\}_{k=1}^n)$ 是任意带标志点的划分, 则
\[
...
对于 Lebesgue 积分, 这个结论可以放宽为\alert{几乎处处} $x \in [a, b],$ 有 $f(x) \leqslant g(x).$
\end{remark}
...
\begin{frame}
\frametitle{Riemann 积分关于被积函数的绝对可积性}
...
\startproof[bg=yellow, fg=red]
我们通过振幅来证明 $|f| \in \mathfrak{R}[a, b].$ 对任意闭区间 $[c, d] \subseteq [a, b],$ 有
\[
...
\begin{frame}
\frametitle{Riemann 积分关于积分区间的可加性}

\begin{prop}[区间可加性]
设 $f \in \mathfrak{R}[a, b],$ 且 $c \in (a, b),$ 则 $f$ 在 $[a, c]$ 和 $[c, b]$ 上均 Riemann 可积, 且有
\[
...
\startproof[bg=yellow, fg=red]
一般地, 设 $[c, d] \subseteq [a, b],$ 则对任意划分 $P = \{x_i\}_{i=0}^n,$ 可添加点 $c$ 和 $d$ 得到划分 $P^* = \{x_i^*\}_{i=0}^{n^*},$ 则有振幅的关系
\[
\sum_{i=1}^{n^*} \omega(f; [x_{i-1}^*, x_i^*]) \Delta x_i^* \leqslant \sum_{i=1}^n \omega(f; [x_{i-1}, x_i]) \Delta x_i.
\]
...
\startproof[bg=yellow, fg=red]
任取带标志点的划分 $(P, \xi) = (\{x_k\}_{k=0}^n,\ \{\xi_k\}_{k=1}^n),$ 有
\[
...
\]
即可. 事实上, (这里要用到一些额外知识: 若函数 Riemann 可积, 则它 Lebesgue 可积, 且积分值相等. 以下论述都是基于 Lebesgue 积分的.) 若 $\lVert f \lVert_p = 0,$ 则 $|f(x)|^p = 0$ 几乎处处成立, 从而 $f(x) = 0$ 几乎处处成立, 进而 $|f(x) g(x)| = 0$ 几乎处处成立, 故 $\lVert fg \lVert_1 = 0.$
...
\startproof[bg=yellow, fg=red]
$p = 1, \infty$ 以及 $\lVert f + g \rVert_p = 0$ 都是简单的情形. 下设 $1 < p < \infty$ 且 $\lVert f + g \rVert_p > 0.$ 记 $q$ 为 $p$ 的共轭指标, 即 $\dfrac{1}{p} + \dfrac{1}{q} = 1.$ 则由 Hölder 不等式, 有
\begin{align*}
...
(\text{\color{red}Hölder 不等式}) & ~ {\color{red}\leqslant} ~ \lVert |f + g |^{p - 1} \rVert_q \cdot \lVert f \rVert_p + \lVert |f + g |^{p - 1} \rVert_q \cdot \lVert g \rVert_p \\
& = \lVert f + g \rVert_p^{p - 1} \cdot \lVert f \rVert_p + \lVert f + g \rVert_p^{p - 1} \cdot \lVert g \rVert_p.
\end{align*}
两边除以 $\lVert f + g \rVert_p^{p - 1}$ 即得所需不等式.
\qed
...
\textbf{(\ref{eq:star2}) $\implies$ (\ref{eq:star}):} 设 $g$ 在 $[a, b]$ 上单调不减 (单调不增类似), 令 $G(x) = g(b) - g(x),$ 则 $G$ 在 $[a, b]$ 上非负不增, 由式~(\ref{eq:star2}), 存在 $\xi \in [a, b],$ 使得
\[
\int_a^b f(x) G(x) ~ \mathrm{d} x = G(a) \int_a^\xi f(x) ~ \mathrm{d} x.
\]
...
\textbf{(\ref{eq:star}) $\implies$ (\ref{eq:star2}):} 由 $g(a) \int_a^\xi f(x) ~ \mathrm{d} x + g(b) \int_\xi^b f(x) ~ \mathrm{d} x = g(a) \cdot \left( \int_a^\xi f(x) ~ \mathrm{d} x + \int_\xi^b \frac{g(b)}{g(a)} f(x) ~ \mathrm{d} x \right),$ 令 $\tau = \frac{g(b)}{g(a)},$ 由于 $g$ 非负不增, 故 $0 \leqslant \tau \leqslant 1.$ 那么
\[
\int_a^t f(x) ~ \mathrm{d} x - \left( \int_a^\xi f(x) ~ \mathrm{d} x + \tau \int_\xi^b f(x) ~ \mathrm{d} x \right) = {\color{red}\int_\xi^t f(x) ~ \mathrm{d} x} - {\color{cyan}\tau \int_\xi^b f(x) ~ \mathrm{d} x}.
\]
...
\alert{注意}, 这里用到了一个结论: $\int_\xi^t f(x) ~ \mathrm{d} x$ 关于 $t$ 是连续的, 这个结论可以利用 Riemann 可积函数的有界性直接证明:
\pause
$\left\lvert \int_{t}^{t+\Delta t} f(x) ~ \mathrm{d} x \right\rvert \leqslant M |\Delta t|,$ 其中 $M = \sup_{x \in [a, b]} |f(x)|.$
...
& \leqslant M \sum_{i=1}^n \int_{x_{i-1}}^{x_i} \omega(g; [x_{i-1}, x_i]) ~ \mathrm{d} x = M \sum_{i=1}^n \omega(g; [x_{i-1}, x_i]) \Delta x_i \xrightarrow{\lambda(P) \to 0} 0.
\end{align*}
...
\begin{frame}
\frametitle{积分第二中值定理的证明 (续)}

接下来证明, $\exists ~ \xi' \in [a, b],$ 使得 $\Circled{1} \xrightarrow{\lambda(P) \to 0} g(a) \int_a^{\xi'} f(x) ~ \mathrm{d} x.$ 记 $F(x) = \int_a^x f(t) ~ \mathrm{d} t,$ 那么
\begin{align*}
\Circled{1} & = \sum_{i=1}^n g(x_{i-1}) \left( \int_a^{{\color{red}x_i}} f(x) ~ \mathrm{d} x - \int_a^{{\color{red}x_{i-1}}} f(x) ~ \mathrm{d} x \right) = \sum_{i=1}^n g(x_{i-1}) (F(x_i) - F(x_{i-1})) \\
& = \sum_{i=1}^n F(x_i) g(x_{i-1}) - \sum_{{\color{red}i=2}}^n F(x_{i-1}) g(x_{i-1}) = \sum_{i=1}^n F(x_i) g(x_{i-1}) - \sum_{{\color{red}i=1}}^{{\color{red}n-1}} F(x_i) g(x_i) \\
& = F(b) g(x_{n-1}) + \sum_{i=1}^{n-1} F(x_i) (g(x_{i-1}) - g(x_i)).
\end{align*}

\pause

显然 $F(x)$ 在 $[a, b]$ 上有界, 令 $d \leqslant F(x) \leqslant D.$ 注意到 $g(x)$ 是非负不增的, 由上式有
\[
{\only<3-> {d \cdot g(a) = ~}} d \cdot g(x_{n-1}) + \sum_{i=1}^{n-1} d (g(x_{i-1}) - g(x_i)) \leqslant \Circled{1} \leqslant D \cdot g(x_{n-1}) + \sum_{i=1}^{n-1} D (g(x_{i-1}) - g(x_i)) {\only<3-> {~ = D \cdot g(a)}}.
\]

\only<4->{
对上式关于 $\lambda(P) \to 0$ 取极限, 可得
\[
d \cdot g(a) \leqslant \lim_{\lambda(P) \to 0} \Circled{1} \leqslant D \cdot g(a).
\]
}
...
对 $\int_a^b f(x) g(x) ~ \mathrm{d} x = \Circled{1} + \Circled{2}$ 关于 $\lambda(P) \to 0$ 取极限:
\[
\int_a^b f(x) g(x) ~ \mathrm{d} x = \lim_{\lambda(P) \to 0} \Circled{1} + \lim_{\lambda(P) \to 0} \Circled{2} = \lim_{\lambda(P) \to 0} \Circled{1} \in [d \cdot g(a), D \cdot g(a)].
\]
于是, 存在 $\eta \in [d, D],$ 使得 $\int_a^b f(x) g(x) ~ \mathrm{d} x = \eta \cdot g(a).$

\pause
\vspace{0.5em}

再次利用 $F(x) = \int_a^x f(t) ~ \mathrm{d} t$ 的连续性, 存在 $\xi' \in [a, b],$ 使得 $\eta = F(\xi') = \int_a^{\xi'} f(x) ~ \mathrm{d} x,$ 即有式~(\ref{eq:star2}) 成立:
\[
\int_a^b f(x) g(x) ~ \mathrm{d} x = g(a) \int_a^{\xi'} f(x) ~ \mathrm{d} x.
\]
\qed
