...
\begin{block}{变上限积分}
$f(x) \in \mathcal{R}[a, b],$ 变上限积分 $F(x) = \int_a^x f(t) ~ \mathrm{d} t,$ $x \in [a, b].$
\end{block}
...
简单来说, 就是有集合 (空间) 的包含关系:
\begin{multline*}
\{\text{Abel 可和级数}\} {\color{magenta} \supsetneqq } \cdots \supsetneqq \{(c,r) ~ \text{可和级数}\} \\
\supsetneqq \cdots \supsetneqq \{(c,1) ~ \text{可和级数}\} \supsetneqq \{\text{通常可和级数}\}
\end{multline*}
