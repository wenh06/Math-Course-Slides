...
\begin{Def}[一致收敛]
设函数项级数 $\sum_{n=1}^\infty u_n(x)$ 的和函数 $S(x)$ 在 $D$ 上有定义. 若对任意 $\varepsilon > 0,$ 存在 {\color{red}只与 $\varepsilon$ 有关的} $N \in \mathbb{N},$ 使得对任意 $n > N$ 和 {\color{red}任意 $x \in D$} 都有
...
\begin{frame}{Weierstraß 判别法 (M-判别法)}
...
\item 结合即得结论. 且在端点 $x = \pm 1$ 处级数收敛 (Leibniz), 利用 Abel 第二定理可延拓.
\end{itemize}
\qed
...
\begin{attnblock}
仅有 $S_n$ 和 $S$ 的连续性, 推不出{\color{red}闭区间上}的一致收敛性 (但可推出{\color{red}内闭}一致收敛性). 单调性是关键的正则性条件.
\end{attnblock}
...
\item 或用 Cauchy-Hadamard: $\varlimsup \frac{n}{\sqrt[n]{n!}} = e,$ 故 $R = 1/e.$
...
\item Cauchy-Hadamard 定理{\color{red}强于} d'Alembert 判别法: 反例 — $a_n$ 取 $2^k$ 当 $k^2 \leqslant n < (k+1)^2,$ 则 $\varlimsup \left\lvert \frac{a_{n+1}}{a_n} \right\rvert = 2,$ 但 $\varlimsup \nroot[n]{|a_n|} = 1,$ 收敛半径 $R = 1 \neq 1/2.$
\end{itemize}
...
(1+x)^\alpha = \displaystyle\sum_{n=0}^\infty C_\alpha^n x^n, & R = 1,\ D \text{ 取决于 } \alpha \\[1.5em]
...
Cauchy 判别法 & $\varlimsup_{n\to\infty} \nroot[n]{a_n}$ & $\longrightarrow$ Cauchy-Hadamard (收敛半径) \\[0.5em]
d'Alembert 判别法 & $\varlimsup_{n\to\infty} a_{n+1}/a_n$ & $\longrightarrow$ d'Alembert (收敛半径) \\[0.5em]
