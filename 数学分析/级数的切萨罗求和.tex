...
想法: 将 $s_n - \sigma_n$ 与 $\sigma_n - \sigma_m$ 联系上, 并用它控制余项的大小. 假设 $n > m,$ 那么有
\begin{align*}
  \sigma_n - \sigma_m & = \dfrac{1}{n} \sum\limits_{k=1}^n s_k - \dfrac{1}{m} \sum\limits_{k=1}^m s_k \\
  & = \dfrac{1}{n} \sum\limits_{k=1}^n s_k - \dfrac{1}{m} \sum\limits_{k=1}^{{\color{red}n}} s_k + \dfrac{1}{m} \sum\limits_{k=m+1}^{n} s_k \\
  & = \dfrac{m - n}{nm} \sum\limits_{k=1}^n s_k + \dfrac{1}{m} \sum\limits_{k=m+1}^{n} s_k.
\end{align*}

另一方面, 有
$$s_n - \sigma_n = s_n - \dfrac{1}{n} \sum\limits_{k = 1}^n s_k = \dfrac{1}{n} \sum\limits_{k = 1}^n (s_n - s_k).$$
...
& = \dfrac{1}{n} \sum\limits_{k = 1}^n (s_n - {\color{red} \cancel{s_k}}) + {\color{red} \cancel{\dfrac{1}{n} \sum\limits_{k=1}^n s_k}} - \dfrac{1}{n-m} \sum\limits_{k=m+1}^{n} s_k \\
...
对于 $\text{\ding{173}} = \dfrac{1}{n-m} \sum\limits_{k=m+1}^{n} (s_n - s_k),$ 我们有估计
{\smaller
\begin{multline*}
...
我们希望有 $\dfrac{n-m-1}{m+1} < \varepsilon,$ 即 $m > \dfrac{n}{1 + \varepsilon} - 1.$ 取
...
取 $N$ 充分大, 使得 $\forall n > m > N$ 都有 $\left\lvert \sigma_n - \sigma_m \right\rvert < \varepsilon^2,$ 那么
...
以上我们证明了两个所谓的Tauber型定理, 即在某些附加的“正则性”条件下，根据考察的量的某些均值的性质，对这个量本身的性质作一些判断。
