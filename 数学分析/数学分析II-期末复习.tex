...
\title[数分II\\
期末复习]{\texorpdfstring{\Huge \bfseries 数学分析II期末复习}{数学分析II期末复习}} % The short title appears at the bottom of every slide, the full title is only on the title page
% \author[\S 11.3\\
...
\item 积分和: $S(f; P; \xi) = \sum\limits_{i=1}^n f(\xi_i) \Delta x_i.$
...
\item 振幅条件增强版 (条件减弱): $\forall ~ \varepsilon > 0,$ 存在划分 $P$ 满足 $\sum\limits_{i=1}^n \omega(f; \Delta_i) \Delta x_i < \varepsilon.$
...
\begin{frame}{定积分区间可加性的一个应用}

\begin{eg}
证明当 $x \to +\infty$ 时, 有 $\int_2^x \frac{\mathrm{d} t}{(\ln t)^2} = O\left(\dfrac{x}{(\ln x)^2}\right).$
\end{eg}
...
当 $x$ 充分大时, 可以将积分区间 $[2, x]$ 分成 $[2, \sqrt{x}]$ 和 $[\sqrt{x}, x]$ 两部分, 则
\begin{align*}
\int_2^x \frac{\mathrm{d} t}{(\ln t)^2} & = \int_2^{\sqrt{x}} \frac{\mathrm{d} t}{(\ln t)^2} + \int_{\sqrt{x}}^x \frac{\mathrm{d} t}{(\ln t)^2} \\
& \leqslant \int_2^{\sqrt{x}} \frac{\mathrm{d} t}{(\ln 2)^2} + \int_{\sqrt{x}}^x \frac{\mathrm{d} t}{(\ln \sqrt{x})^2} \\
& = \frac{\sqrt{x}-2}{(\ln 2)^2} + \frac{\sqrt{x}}{(\ln \sqrt{x})^2} = O\left(\dfrac{x}{(\ln x)^2}\right).
\end{align*}
\qed
...
\item $n = 4$ (Cotes 公式):
\[
\int_a^b f(x) ~ \mathrm{d} x \approx \dfrac{b-a}{90} \left( 7f(a) + 32f\left( \dfrac{3a+b}{4} \right) + 12f\left( \dfrac{a+b}{2} \right) + 32f\left( \dfrac{a+3b}{4} \right) + 7f(b) \right)
\]
\end{itemize}
...
{\color{red}\ding{43}}定积分可以推广到反常积分的性质:
\begin{itemize}
\item 线性性.
\item 保序性.
\item 区间可加性.
...
{\color{red}\ding{43}}\textbf{不可以}推广到反常积分的性质:
\begin{itemize}
\item 乘积可积性.
\pause
...
\[f(x) \in \mathfrak{R}[a, b] ~~ \substack{\implies \\ \cancel\Longleftarrow} ~~ \lvert f(x) \rvert \in \mathfrak{R}[a, b]\]
在反常积分中正好反过来了:
\[\int_a^\omega \lvert f(x) \rvert ~ \mathrm{d} x ~ \text{ 收敛} ~~ \substack{\implies \\ \cancel\Longleftarrow} ~~ \int_a^\omega f(x) ~ \mathrm{d} x ~ \text{ 收敛}.\]
\end{attnblock}
...
\item $g(x)$ 在 $[a, \omega)$ 上单调有界;
\end{itemize}
\pause
\vspace{0.5em}
\item Dirichlet 判别法:
\begin{itemize}
\item $\int_a^A f(x) ~ \mathrm{d} x$ 关于 $A \in [a, \omega)$ 有界,
\item $g(x)$ 在 $[a, \omega)$ 上单调趋于 $0$ ($\lim\limits_{x \to \omega} g(x) = 0$).
\end{itemize}
\end{itemize}
\end{thm}
...
\pause
若部分积序列 $P_n = \prod\limits_{k=1}^n p_k, n = 1, 2, \dots$ 收敛到一个{\color{red}非零实数}, 则称无穷乘积 $\prod\limits_{n=1}^\infty p_n$ 收敛, 且有
\[\prod\limits_{n=1}^\infty p_n = \lim\limits_{n\to\infty} \prod\limits_{k=1}^n p_k.\]
否则称无穷乘积 $\prod\limits_{n=1}^\infty p_n$ 发散.
\end{Def}
...
\begin{frame}
\frametitle{无穷乘积收敛性的判别 | 典型例子}

\Circled{I} 的典型例子 (仅 $\sum_{n=1}^\infty a_n$ 收敛)
\[
...
\Circled{II} 的典型例子 (仅 $\sum_{n=1}^\infty a_n^2$ 收敛)
\[
a_n = \frac{1}{n}.
\]
...
\Circled{III} 的典型例子 (仅 $\prod_{n=1}^\infty (1 + a_n)$ 收敛)
\[a_n = \begin{cases}
...
\begin{frame}{无穷乘积的经典例子}

\begin{eg}[$\sin x$ 的无穷乘积展开 (Euler)]
\[\frac{\sin \pi x}{\pi x} = \prod_{n=1}^\infty \left( 1 - \frac{x^2}{n^2} \right), \quad x \in \mathbb{R}.\]
\end{eg}

\pause
\vspace{0.5em}

{\color{red}技巧}: 对无穷乘积取{\color{red}对数导数} $\frac{\mathrm{d}}{\mathrm{d}x} \ln(\cdot)$ 可将乘积转化为级数，是处理无穷乘积展开的重要方法:
\[
\cot \pi x = \frac{\cos \pi x}{\sin \pi x} =
\frac{1}{\pi x} - \frac{1}{\pi} \sum_{n=1}^\infty \frac{2x}{n^2} \cdot \frac{1}{1 - \frac{x^2}{n^2}}
= \frac{1}{\pi x} + \frac{1}{\pi} \sum_{n=1}^\infty \frac{2x}{x^2 - n^2}
\]
由此可得到一系列的特殊的数项级数求和的计算方法, 例如 $\sum\limits_{n=1}^\infty \dfrac{1}{n^2+1}.$
\pause
或者, 在上式中展开 $\frac{1}{1 - \frac{x^2}{n^2}}$ 可得由 $\zeta$ 函数表达的 $\tan x$ 的幂级数
\[
\tan x = \cot x - 2 \cot 2x = \sum_{n=1}^\infty \frac{2 \cdot 4^n (4^n - 1)}{(2\pi)^{2n}} \zeta(2n) \cdot x^{2n-1}
\]
进而通过比较幂级数系数的方法, 计算出 $\zeta$ 函数的一些特殊值 $\zeta(2k)$ 的具体数值.
...
{\smaller[1]
\begin{tabular}{|p{2.8cm}|p{4cm}|p{3.5cm}|p{4cm}|}
\hlineB{3.5}
\bfseries 判别法 & \bfseries 反常积分 & \bfseries 正项/数项级数 & \bfseries 函数项级数 \\
\hline \hline
比较判别法 & $0 \leqslant f(x) \leqslant g(x)$ & $0 \leqslant a_n \leqslant b_n$ & Weierstraß 判别法 \\[0.3em]
Cauchy 判别法 & $x^p f(x) \to \ell$ & $\varlimsup_{n\to\infty} \nroot[n]{a_n}$ & Cauchy-Hadamard (收敛半径) \\[0.3em]
d'Alembert 判别法 & --- & $\varlimsup_{n\to\infty} a_{n+1}/a_n$ & d'Alembert (收敛半径) \\[0.3em]
Raabe 判别法 & --- & $n(a_n/a_{n+1} - 1)$ & --- \\[0.3em]
积分判别法 & (定义本身) & $\int_a^{+\infty} f(x) ~ \mathrm{d}x \leftrightarrow \sum f(n)$ & --- \\[0.3em]
Leibniz 判别法 & --- & 交错级数 & --- \\[0.3em]
A-D 判别法 & $\int f$ 收敛, $g$ 单调有界 ~ 或者 ~ $\int_a^A f$ 有界, $g \searrow 0$ & 一般级数 A-D & 函数项级数 A-D \\
\hlineB{3.5}
\end{tabular}
}
\end{table}
...
那么称函数项级数 $\sum\limits_{n=1}^\infty a_n(x)$ 在 $D$ 上一致收敛到 $S(x),$ 记作 $\sum\limits_{n=1}^\infty a_n(x) \stackrel{D}{\rightrightarrows}S(x).$
\end{Def}
...
\item Dirichlet 判别法:
\begin{itemize}
\item $S_n(x) := \sum\limits_{k=1}^n b_k(x)$ 在 $D$ 上一致有界;
...
\[\int_a^b S(x) ~ \mathrm{d} x = \lim\limits_{n\to\infty} \int_a^b S_n(x) ~ \mathrm{d} x\]
\end{itemize}
...
\[f(x) = \sum\limits_{n=1}^\infty a^n \varphi(b^n x), ~~ \text{其中} ~ 0 < a < 1, b \in \mathbb{N}, ab \geqslant 1.\]
\end{itemize}
...
幂级数 $\sum\limits_{n=0}^\infty a_n (x-x_0)^n:$ 特殊的函数项级数, 通项是 $x-x_0$ 的单项式. 它的收敛域 (除端点外) 关于 $x_0$ 对称, 有所谓\alert{收敛半径}的概念:
...
\begin{thm}[d'Alembert 判别法]
设 $\sum\limits_{n=0}^\infty a_n x^n$ 满足 $\lim\limits_{n\to\infty} \left\lvert \dfrac{a_{n+1}}{a_n} \right\rvert = A,$ 那么幂级数收敛半径 $R = \dfrac{1}{A}.$
\end{thm}
...
由不等式
\vspace{-0.5em}
\[\varliminf\limits_{n\to\infty} \left\lvert \dfrac{a_{n+1}}{a_n} \right\rvert \leqslant \varliminf\limits_{n\to\infty} \sqrt[n]{\left\lvert a_n \right\rvert} \leqslant {\color{red}\varlimsup\limits_{n\to\infty} \sqrt[n]{\left\lvert a_n \right\rvert}} \leqslant \varlimsup\limits_{n\to\infty} \left\lvert \dfrac{a_{n+1}}{a_n} \right\rvert,\]
若知道 $\varliminf\limits_{n\to\infty} \left\lvert \dfrac{a_{n+1}}{a_n} \right\rvert$ 或 $\varliminf\limits_{n\to\infty} \sqrt[n]{\left\lvert a_n \right\rvert}$ 或 $\varlimsup\limits_{n\to\infty} \left\lvert \dfrac{a_{n+1}}{a_n} \right\rvert,$ 那么只能得到关于收敛半径的不等式估计.

\pause

反过来, 如果知道收敛半径, 那么可以知道 $\varlimsup\limits_{n\to\infty} \sqrt[n]{\left\lvert a_n \right\rvert}$ 的值, 但是不能得到 $\lim\limits_{n\to\infty} \left\lvert \dfrac{a_{n+1}}{a_n} \right\rvert$ 的值, 甚至极限是否存在都不能 (仅依据收敛半径) 下结论.
...
\begin{itemize}[<+->]
\item Lagrange 形式简单, 但 $x < 0$ 时因子 $(\frac{1}{1+\xi})^{n+1}$ 不易处理.
\item Cauchy 形式额外引入了 $(\frac{1-\theta}{1+\theta x})^n \in (0, 1),$ 更易处理.
\end{itemize}
...
\begin{frame}{级数的 Abel 求和法}

\begin{Def}[Abel 求和法]
若 $\lim\limits_{x \to 1-} \sum\limits_{n=1}^\infty a_n x^n = A$ 为一个有限实数, 则称数项级数 $\sum\limits_{n=1}^\infty a_n$ 是 Abel 意义下可和的, 并定义
\[\sum\limits_{n=1}^\infty a_n = \lim\limits_{x \to 1-} \sum\limits_{n=1}^\infty a_n x^n = A.\]
\end{Def}

\pause

若数项级数 $\sum\limits_{n=1}^\infty a_n$ 是 Cesàro 意义下可和的, 那么它也是 Abel 意义下可和的, 且两种求和结果相同:

\begin{thm}[Frobeinus]
若数项级数 $\sum\limits_{n=1}^\infty a_n$ 在 Cesàro $(c, 1)$ 意义的和为 $A \in \mathbb{R},$ 即 $\lim\limits_{n\to\infty} a_n = A ~ (c, 1),$ 那么它在 Abel 意义下的和也为 $A.$
\end{thm}
...
证明的关键是, 在 $(-1, 1)$ 上恒有:
\begin{itemize}
\item $\sum\limits_{n=1}^\infty a_n x^n = (1 - x)^2 \sum\limits_{n=1}^\infty n \sigma_n x^n;$
\item $1 = (1 - x)^2 \sum\limits_{n=1}^\infty (n+1) x^n.$
\end{itemize}
...
简单来说, 就是有集合 (空间) 的包含关系:
\[
\{\text{Abel 可和级数}\} {\color{magenta} \supsetneqq } \cdots \supsetneqq \{(c,r) ~ \text{可和级数}\} \supsetneqq \cdots \supsetneqq \{(c,1) ~ \text{可和级数}\} \supsetneqq \{\text{通常可和级数}\}
\]
...
幂级数部分和是自然的候选, 但要求函数{\color{red}解析}, 且形式固定 ($P_n - P_{n-1} = a_n (x-x_0)^n$).
...
\begin{frame}{Bernstein 多项式与 Korovkin 定理}
...
设 $\{L_n\}$ 是 $C[a,b]$ 上一列{\color{red}正线性算子}, 且对 $i = 0, 1, 2$ 有 $L_n(t^i; x) \stackrel{[a,b]}{\rightrightarrows} x^i,$
则 $\forall~f \in C[a,b],$ $L_n(f; x) \stackrel{[a,b]}{\rightrightarrows} f(x).$
\end{thm}

\pause

Weierstraß 第一逼近定理通过变量代换 $g(z) = f((b-a)z + a)$ 从 Bernstein 定理推出.
...
\[
\begin{array}{ll}
e^x = \displaystyle\sum_{n=0}^\infty \dfrac{x^n}{n!}, & R = +\infty \\[1.2em]
\sin x = \displaystyle\sum_{k=0}^\infty \dfrac{(-1)^k}{(2k+1)!} x^{2k+1}, & R = +\infty \\[1.2em]
\cos x = \displaystyle\sum_{k=0}^\infty \dfrac{(-1)^k}{(2k)!} x^{2k}, & R = +\infty \\[1.2em]
\ln(1+x) = \displaystyle\sum_{n=1}^\infty \dfrac{(-1)^{n-1}}{n} x^n, & R = 1,\ D = (-1, 1] \\[1.2em]
(1+x)^\alpha = \displaystyle\sum_{n=0}^{\infty \text{ or } \alpha} C_\alpha^n x^n, & R = 1 \text{ or } \infty,\ D \text{ 取决于 } \alpha \\[1.2em]
\arctan x = \displaystyle\sum_{n=0}^\infty \dfrac{(-1)^n}{2n+1} x^{2n+1}, & R = 1,\ D = [-1, 1] \\[1.2em]
\arcsin x = \displaystyle\sum_{n=0}^\infty \dfrac{(2n-1)!!}{(2n)!!} \cdot \dfrac{x^{2n+1}}{2n+1}, & R = 1,\ D = [-1, 1] \\[1.2em]
...
{\color{red}\ding{43}} 思考: 若 $F$ 仅为闭集 (不一定有界), 连续映射下 $f(F)$ 是否仍为闭集?
