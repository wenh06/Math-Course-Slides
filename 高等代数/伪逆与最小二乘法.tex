...
\end{gather*}
\end{block}
于是 $\operatorname{id} = \operatorname{pr}_1 \Otop \operatorname{pr}_2,$ 进而任何一个线性映射 $f: \mathbb{R}^k \to \mathbb{R}^m$ 都可以写成如下的形式:
\[
f = f_1 \Otop f_2 = (\operatorname{pr}_1 \circ f) \Otop (\operatorname{pr}_2 \circ f): \mathbb{R}^k \to \mathbb{R}^m \xrightarrow{\operatorname{pr}_1 \Otop \operatorname{pr}_2} C(A) \Otop N(A^T).
\]
\alert{注意, 这里并不是说 $f$ 的值域 $f(\mathbb{R}^k)$ 包含 $C(A)$ 或者 $N(A^T)$ 的子空间, 而是说 $f$ 的值域中的每一个元素都可以唯一分解成 $C(A)$ 中的一个元素与 $N(A^T)$ 中的一个元素的和.}
我们采用同样的记号, 记 $g_1: W \to \mathbb{R}^m,$ $g_2: W^\perp \to \mathbb{R}^m$ 得到的线性映射为
\[
...
满足
\begin{itemize}
\item $\tau|_{C(A)} = \tilde{\sigma}^{-1} \Otop \eta$, 其中 $\eta: C(A) \to N(A)$ 是任意一个线性映射;
\item 对左零空间 $N(A^T)$ 中的任意 $\mathbf{y}$ 映射过去的像没有任何要求, $A^-\mathbf{y}$ 可以任取 (保证是线性映射即可).
\end{itemize}
综合起来, 就是
\[
\tau = (\tilde{\sigma}^{-1} ~ {\color{red}\Otop} ~ \eta) ~ {\color{cyan}\Otop} ~ \zeta : C(A) ~ {\color{cyan}\Otop} ~ N(A^T) \to \mathbb{R}^n = C(A^T) ~ {\color{red}\Otop} ~ N(A),
\]
...
齐次线性方程组 $A\mathbf{x} = \mathbf{0}$ 的解的全体就是 $N(A) = \ker \sigma$. 之前我们已经知道, $\tau = (\tilde{\sigma}^{-1} \Otop \eta) \Otop \zeta,$ 其中 $\eta: C(A) \to N(A)$ 是任意一个线性映射, $\zeta: N(A^T) \to \mathbb{R}^n$ 是任意一个线性映射. 考虑复合映射
\[
\xymatrix@R=0.3em{
C(A^T) \Otop N(A)
  \ar[r]^-{\sigma}
...
\sigma(\mathbf{z}_1)+\mathbf{0}
  \ar@{|->}[r]
&
(\tilde{\sigma}^{-1}\Otop\eta)(\sigma(\mathbf{z}_1))
+\zeta(\mathbf{0})
}
\]
可知 $(\tau \circ \sigma)(\mathbf{z}_1 + \mathbf{z}_2) = (\tilde{\sigma}^{-1}\Otop\eta)(\sigma(\mathbf{z}_1)) = \mathbf{z}_1 + \eta(\sigma(\mathbf{z}_1))$. 于是
\[
...
还是基于之前的分解 $\tau = (\tilde{\sigma}^{-1} \Otop \eta) \Otop \zeta$, 其中 $\eta: C(A) \to N(A)$ 是任意一个线性映射, $\zeta: N(A^T) \to \mathbb{R}^n$ 是任意一个线性映射. 记 $\mathbf{b} = \mathbf{b}_1 + \mathbf{b}_2,$ 其中 $\mathbf{b}_1 \in C(A), \mathbf{b}_2 \in N(A^T)$. 那么有
\[
AA^-\mathbf{b} = \sigma \circ \tau(\mathbf{b}_1 + \mathbf{b}_2) = \sigma \circ \tau(\mathbf{b}_1) + \sigma \circ \tau(\mathbf{b}_2) = \sigma \circ (\tilde{\sigma}^{-1} \Otop \eta)(\mathbf{b}_1) + \sigma \circ \zeta(\mathbf{b}_2) = \mathbf{b}_1 + \underbrace{\sigma(\zeta(\mathbf{b}_2))}_{\in C(A)}.
\]
