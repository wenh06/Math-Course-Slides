...
\author[SVD] % Your institution as it will appear at the bottom of every slide, may be shorthand to save space
{\Large \bfseries 奇异值分解}
...
\begin{thm}[奇异值分解定理, Theorem of Singular Value Decomposition]
...
\label{eq2}
\end{equation*}
又由 \eqref{eq1}~式知 $M^T\mathbf{x}$ 不是零向量, 故 $\lambda$ 也是 $M^TM$ 的正特征值.
\qed
...
把 $\left\{ \mathbf{u}_1,\ldots,\mathbf{u}_r \right\}$ 扩充为 $\mathbb{R}^m$ 的一组单位正交基 $\left\{ \mathbf{u}_1,\ldots,\mathbf{u}_r,  \mathbf{u}_{r+1},\ldots,\mathbf{u}_m \right\}$, 那么我们就得到了 SVD 定理中的形式:
...
\begin{frame}
\frametitle{奇异值分解应用: 图像压缩}

利用 SVD, 我们可以把一个秩为 $r$ 的 $m\times n$ 矩阵 $M$ 写作
...
还需要强调的是, 计算机中 SVD 的实现, 并不是按我们证明 SVD 定理中的步骤来的, 而是用另外的快速的算法.  (类似的还有线性代数中其他很多的计算)
