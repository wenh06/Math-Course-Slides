...
\begin{itemize}
\item 结论:
\item \quanhao{(i)} 任意有限扩张 $E/F$ 为 Galois 扩张,
  而且 $\mathrm{Gal}(E/F) = \langle \tau \rangle$,
  其中 $\tau(\alpha) = \alpha^{p^n}$ 为 $E/F$ 上 Frobenius 自同构;
\item \quanhao{(ii)} $F_p$ 为 $p$ 元有限域, $\bar{F}_p$ 为 $F_p$ 的代数闭包.
...
\begin{itemize}
\item \quanhao{①} $\mathbb{R}$ 上没有奇数次扩张;
\item \quanhao{②} $\mathbb{C}$ 上不存在 2 次扩张;
\item \quanhao{③} $E/\mathbb{C}$ 为有限次扩张 $\Longrightarrow$ $E = \mathbb{C}$.
\end{itemize}
\end{eg}
...
$[F:\mathbb{R}] = \frac{|G|}{|H|}$, 由 \quanhao{①} 知此时 $\mathbb{R}$ 有 $F = \mathbb{R}$,
即 $H = G$, 即 $|G| = 2^n$.
...
\startproof[bg=yellow, fg=red](③ 的证明, 续)
若 $m = 1$, 则与 \quanhao{②} 的结论矛盾.
若 $m > 1$, 令 $H_1$ 为 $G_1$ 的一个阶为 $2^{m-1}$ 阶子群,
令 $F_1 = I_n(H_1)$, 则 $\mathbb{C} \subset F_1 \subset L$,
$[F_1:\mathbb{C}] = \frac{|G_1|}{|H_1|} = 2$, 也与 \quanhao{②} 矛盾.
