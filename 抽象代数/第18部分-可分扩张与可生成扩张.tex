...
可分生成扩张的注记:
\begin{itemize}
\item \quanhao{①} 若 $\chi(F) = 0$, 则 $\forall\ E/F$ 为可分扩张,
  也是可分生成扩张;
\item \quanhao{②} 若 $E/F$ 为代数扩张, 则 $E/F$ 为可分扩张
  $\iff$ $E/F$ 为可分生成扩张;
\item \quanhao{③} 若 $E/F(S)$, $S$ 为 $F$ 上代数无关集, 即 $E/F$
...
\quanhao{①} 任一域 $F$ 上线性空间 $V$ 的基存在;
\quanhao{②} 若 $E$ 与 $D$ 在 $F$ 上线性不相交, 则 $E/F$ 的一组基,
也是 $E \cdot D/D$ 的一组基;
\quanhao{③} 若 $E$ 与 $L$ 在 $F$ 上线性不相交, 则 $E \cdot D/F$ 的
线性无关组也是 $L$ 上的线性无关组;
\quanhao{④} 由 Zorn 引理可证明.
...
\quanhao{⑤} 若 $A$ 为 $E/F$ 的一组基, 那么 $\forall\ \alpha \in D[E]$,
...
\startproof[bg=yellow, fg=red]((i) $\Longrightarrow$ (iii) 的证明, 续)
由 \quanhao{①} 可知, $\exists\ i$, $1 \leq i \leq m+1$, 有
...
\quanhao{①} 若 $F \subset E \subset K$ 为域扩张, $E/F$ 为代数扩张,
...
\quanhao{③} $F \subset E \subset K$, 若 $E/F$ 为可分代数扩张,
...
为纯不可分代数扩张,
而 $L/L(\alpha_1, \dots, \alpha_m)$ 为可分代数扩张, 由 \quanhao{③} 知
