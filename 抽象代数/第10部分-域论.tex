...
\begin{itemize}
\item 结论:
\item \quanhao{①} $\mathbb{Q}$ 与 $F_p$ 为最小的域, 称为素域;
\item \quanhao{②} 任意一个域都为在 $\mathbb{Q}$ 上或 $F_p$ 上扩张而得的.
\end{itemize}
...
\begin{itemize}
\item 注:
\item \quanhao{①} 代数元素 $\alpha$ 的极小多项式 $f(x)$ 为 $F[x]$ 中的
  不可约多项式, 且 $N = \langle f(x) \rangle$ 为 $F[x]$ 中的一个极大理想;
\item \quanhao{②} $\{\alpha_1, \dots, \alpha_n\}$ 在 $F$ 上代数无关
  $\Longrightarrow$ $\forall\ \alpha_i$ 为 $F$ 上的超越元素; 反之不成立;
\item \quanhao{③} 线性相关为代数相关特例 (多项式为一次的多项式).
\end{itemize}
...
\begin{itemize}
\item \quanhao{①} 任一有限扩张 $E/F$ 为代数扩张;
\item \quanhao{②} 若 $F \subset E \subset L$, $E/F$, $L/E$ 都为有限扩张, 那么
  $L/F$ 也是有限扩张, 且
...
\begin{itemize}
\item \quanhao{①} $\{ 1, \bar{x}, \dots, \overline{x^{n-1}} \}$
  为 $E/F$ 中线性无关组;
\item \quanhao{②} $\forall\ E$ 中元素可由
