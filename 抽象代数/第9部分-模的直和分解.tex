...
\begin{itemize}
\item \quanhao{①} $e_i^2 = e_i$, $1 \leq i \leq n$;
\item \quanhao{②} $e_i e_j = 0$, 若 $i \neq j$;
\item \quanhao{③} $\sum_i e_i = 1_M$.
\end{itemize}
...
\begin{itemize}
\item 结论:
\item \quanhao{①} 有限长度模, 强不可分解 $\Longleftarrow$ 不可分解;
\item \quanhao{②} 有限长度模 Krull--Schmidt 分解定理.
\end{itemize}
...
\quanhao{①} $M = \sigma^n(M) + \ker(\sigma^n)$;
\quanhao{②} $\sigma^n(M) \cap \ker(\sigma^n) = 0$.

\pause

对\quanhao{②}: $\forall\ \alpha \in \sigma^n(M) \cap \ker(\sigma^n)$,
...
\startproof[bg=yellow, fg=red](\quanhao{①} 的证明)
