...
\begin{empheq}[box={\fancymath[colback=cyan!30, sharp corners]}]{align*}
  ...
  & \only<3->{\text{\color{red!80}\ding{43} 柯西判别法}} \\
  & \only<4->{\text{\color{red!80}\ding{43} 达朗贝尔判别法}} \\
  & \only<5->{\text{\color{red!80}\ding{43} 拉比判别法}} \\
...
\only<8->{
\begin{center}
我们只能从最基本的{\color{red}柯西收敛原理}出发, 进行研究. 
\end{center}
}
...
\section[柯西\\收敛原理]{柯西收敛原理}

%------------------------------------------------

\begin{frame}{级数的柯西收敛原理}
...
\begin{attnblock}
\begin{itemize}
\item[\ding{43}] 柯西收敛原理对一般的度量空间都是成立的
\pause
...
\begin{Def}[莱布尼茨级数]
如果级数$\sum\limits_{n=1}^\infty x_n = \sum\limits_{n=1}^\infty (-1)^{n+1} u_n ~ (u_n > 0)$为交错级数, 并且数列$\{u_n\}$单调减少且收敛于$0,$ 则称此级数为\textbf{莱布尼茨级数}. 
\end{Def}

\pause

\begin{block}{莱布尼茨级数的一些例子}
\vspace{-1.5em}
...
可知$\sum\limits_{n=1}^\infty \sin \left( \sqrt{n^2+1} \pi \right)$是莱布尼茨级数. 
...
\section[A-D判别法]{阿贝尔判别法与狄利克雷判别法}

%------------------------------------------------

\begin{frame}{回忆 | {\smaller[2] 反常积分的A-D判别法}}

我们先回忆一下一般函数的反常积分敛散性的阿贝尔判别法与狄利克雷判别法 (A-D判别法)：
...
对于一般的情况, 目前能用的工具只有柯西收敛原理, 所以要考察
$$\left\lvert \sum\limits_{k=n+1}^{m} a_k b_k \right\rvert.$$

\pause

(1). 阿贝尔判别法：条件为$\sum\limits_{n=1}^{\infty} b_n$收敛, $a_n$单调有界. 
\vspace{-1em}
\begin{align*}
...
(2). 狄利克雷判别法：条件为$\sum\limits_{n=1}^{N} b_n$关于$N$有界, $a_n$单调趋于$0.$
\vspace{-1em}
\begin{align*}
\sum\limits_{n=1}^{\infty} a_n ~ \text{单调趋于$0$} ~~ & {\color{red}\Longrightarrow} ~~
...
\pause
\begin{empheq}[box={\fancymath[colback=cyan!30, drop lifted shadow, sharp corners]}]{equation*}
  \text{\bfseries\color{red!80}莱布尼茨级数： $\sum\limits_{n=1}^{\infty} (-1)^{n+1} u_n, ~~ 0 < u_n \text{ 单调减小, 且 } u_n \to 0.$}
\end{empheq}

\pause

若令$b_n = (-1)^{n+1}, a_n = u_n,$ 那么它们满足狄利克雷判别法的条件
\begin{empheq}[box={\fancymath[colback=cyan!30, drop lifted shadow, sharp corners]}]{equation*}
...
也就是说, 莱布尼茨判别法可以由狄利克雷判别法导出, 可视为是狄利克雷判别法的一个特例. 
...
2. 阿贝尔判别法可以由狄利克雷判别法导出：

\pause
\vspace{0.8em}

事实上, 若阿贝尔判别法的条件成立：$\sum\limits_{n=1}^{\infty} b_n$收敛, $a_n$单调有界. \par
由数列的单调有界定理, $a_n$收敛于某个实数$a = \lim\limits_{n\to\infty} a_n.$ 那么$c_n : = a_n - a$就构成了一个单调趋于$0$的数列. 

\pause
\vspace{0.8em}

另一方面, 由于$\sum\limits_{n=1}^{\infty} b_n$收敛, 那么$\sum\limits_{n=1}^{N} b_n$显然关于$N$有界. 于是, 根据狄利克雷判别法, $\sum\limits_{n=1}^{\infty} c_nb_n$收敛, 从而知
...
\textbf{证明：} $\{a_n\}$单调趋于$0,$ 这一条件是狄利克雷判别法的一半, 自然希望证明另一半条件, 即$\sum\limits_{n=1}^N \sin nx$关于$N$有界. 
...
% & = \sin \dfrac{1}{2}x \sin x + \cos \dfrac{1}{2}x \sum\limits_{k=2}^{N} \left( \cos (k-1)x - \cos kx \right) + \sin \dfrac{1}{2}x \sin Nx \\
& = \sin \dfrac{1}{2}x \sin x + \cos \dfrac{1}{2}x \cos x - \cos \dfrac{1}{2}x \cos Nx + \sin \dfrac{1}{2}x \sin Nx \\
...
对于固定的$x,$ 上式关于$N$显然是有界的, $\dfrac{1}{\left\lvert \sin\frac{x}{2} \right\rvert}$即为它的一个界. 于是, 根据狄利克雷判别法, $\sum\limits_{n=1}^\infty a_n \sin nx$收敛. 
...
{\larger[2] \color{red} \ding{43}} 级数的柯西收敛原理 (充要条件)：
$$\left\lvert x_{n+1} + x_{n+2} + \cdots + x_m \right\rvert = \left\lvert \sum\limits_{k=n+1}^m x_k \right\rvert < \varepsilon$$

\pause

{\larger[2] \color{red} \ding{43}} 莱布尼茨判别法：莱布尼茨级数都收敛. 
$$\text{莱布尼茨级数：} \sum\limits_{n=1}^{\infty} (-1)^{n+1} u_n, ~~ 0 < u_n \text{ 单调减小, 且 } u_n \to 0.$$

\pause

{\larger[2] \color{red} \ding{43}} A-D判别法 ($\sum\limits_{n=1}^{\infty} a_n b_n$收敛的充分条件)：
\begin{align*}
\text{阿贝尔判别法：} & \sum\limits_{n=1}^{\infty} b_n ~ \text{收敛}, ~ a_n ~ \text{单调有界} \\
\text{狄利克雷判别法：} & \sum\limits_{n=1}^{N} b_n ~ \text{关于$N$有界}, ~ a_n ~ \text{单调趋于$0$}
\end{align*}
