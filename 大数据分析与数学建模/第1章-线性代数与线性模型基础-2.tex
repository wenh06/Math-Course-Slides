...
\begin{alertblock}{结论}
这再次得到了\textbf{正规方程}！
无需复杂的微积分求导，仅凭几何直觉 ($e \perp X$) 就能得到相同的结果。
\end{alertblock}
...
在矩阵形式下，令 $\mathbf{1}$ 为全1向量，$\bar{y}\mathbf{1}$ 为均值向量。因为模型包含截距项，残差 $\mathbf{e}$ 与 $\mathbf{1}$ 正交，可推导得：
\[
...
(Intercept)   101.00      1.000  101.000 0.000097
breedB          9.00      1.414    6.364 0.023800
breedC          4.00      1.732    2.309 0.147200
\end{verbatim}
...
\begin{itemize}
    \item $R^2 \in [0, 1]$。
    \item $R^2$ 表示响应变量 $y$ 的变异中，有多少比例可以被模型解释。
    \item 例如 $R^2 = 0.8$ 意味着 80\% 的变异来源已查明。
\end{itemize}
...
单纯增加自变量个数 $p$，哪怕是随机数，$R^2$ 也只会增加不会减少。为了公平比较不同模型：
...
\begin{itemize}
    \item 引入了对参数个数 $p$ 的惩罚。
    \item 如果新变量解释力弱，增加它反而会降低 $R^2_{adj}$。
...
构造 $t$ 统计量：
\[
t = \frac{\hat{\beta}_j - 0}{\text{SE}(\hat{\beta}_j)} \sim t(n-p)
\]
其中标准误 $\text{SE}(\hat{\beta}_j)$ 是协方差矩阵 $\hat{\sigma}^2 (X^T X)^{-1}$ 对角线元素的平方根。

若 $|t|$ 很大（或 $p$ 值很小），则拒绝 $H_0$，认为该因素显著。
