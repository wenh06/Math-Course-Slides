...
\item 外测度与可测集类的性质; \alert{环、$\sigma$-环、$\sigma$-代数}与\alert{Carath\'{e}odory 扩张定理}: 测度的自然舞台;
...
\section[$\mathbb{R}^n$ 外测度]{无界集与 $\mathbb{R}^n$ 上的勒贝格外测度}
...
\section[可测集]{Lebesgue 可测集: Carath\'{e}odory 条件}
...
\section[环与 $\sigma$-代数]{环、$\sigma$-环与 $\sigma$-代数}
...
\item $\mathscr{M}$ (可测集类) 正是 $\mathbb{R}^n$ 上的一个 $\sigma$-代数, $m$ 是其上的\alert{可列可加非负函数}, 这就是``($\sigma$-代数上的) 测度''的雏形 (本节末正式化).
\end{itemize}
\end{remark}
...
\section[测度扩张]{环上的测度与 Carath\'{e}odory 扩张定理}

%------------------------------------------------

\begin{frame}{从``体积''到一般测度}

回看本节的机器: \alert{外测度} (覆盖取下确界) $+$ \alert{Carath\'{e}odory 条件} $\Longrightarrow$ 可测集类 $\mathscr{M}$ 与其上的可列可加函数. 整套流程没有用到``矩体体积''的特殊性质: \alert{换上任何环上的测度都照常运转}.

\pause

\begin{Def}[环上的测度]
设 $\mathscr{R}$ 为环, $\mu: \mathscr{R} \to [0, +\infty]$ 满足
\begin{enumerate}
\item $\mu(\emptyset) = 0$;
\item \alert{(可列) 可加性}: 对 $\mathscr{R}$ 中两两不交、并仍在 $\mathscr{R}$ 中的集列 $\{E_k\}$, $\mu\big(\bigcup_k E_k\big) = \sum_k \mu E_k$,
\end{enumerate}
则称 $\mu$ 为 $\mathscr{R}$ 上的\alert{测度}. 若不要求非负性, 则称\alert{广义测度} (有符号; Hahn--Jordan 分解留待后续课程).
\end{Def}

\begin{remark}
(2) 蕴含有限可加性 (令 $E_{n+1} = E_{n+2} = \cdots = \emptyset$).
\end{remark}
...
\begin{frame}{环上测度的性质: 连续性定理}

\begin{thm}[环上测度的性质]
设 $\mu$ 为环 $\mathscr{R}$ 上的测度.
\begin{enumerate}
\item \textbf{单调性}: $E_1 \subset E_2$, $E_i \in \mathscr{R} \implies \mu E_1 \leqslant \mu E_2$;
\item \textbf{半可加性}: $E_k \in \mathscr{R}$ 且 $\bigcup_k E_k \in \mathscr{R} \implies \mu\big(\bigcup_k E_k\big) \leqslant \sum_k \mu E_k$;
\item \textbf{连续性}: 对 $\mathscr{R}$ 中\alert{渐张}列 $\{E_k\}$ 且 $\bigcup_k E_k \in \mathscr{R}$, 有 $\mu\big(\bigcup_k E_k\big) = \lim_k \mu E_k$; 对\alert{渐缩}列且 $\mu E_1 < +\infty$, 有 $\mu\big(\bigcap_k E_k\big) = \lim_k \mu E_k$.
\end{enumerate}
\end{thm}
...
(1)(2) 与 §2.4 外测度情形相同. (3) 渐张: 不交化 $E = E_1 \cup (E_2 \setminus E_1) \cup (E_3 \setminus E_2) \cup \cdots$ (差仍在 $\mathscr{R}$ 中), 由可列可加性与裂项
\[
\mu E = \mu E_1 + \sum_{k \geqslant 1} \mu(E_{k+1} \setminus E_k) = \mu E_1 + \lim_{n} \sum_{k=1}^{n} (\mu E_{k+1} - \mu E_k) = \lim_n \mu E_{n+1};
\]
渐缩: $\bigcap_k E_k = E_1 \setminus \bigcup_k (E_1 \setminus E_k)$, 对渐张列 $\{E_1 \setminus E_k\}$ 用刚证结论与可减性 (需 $\mu E_1 < +\infty$).
\qed
...
\begin{frame}{$\sigma$-环上的外测度}

\begin{Def}[$\sigma$-环上的外测度]
设 $\mathscr{S}$ 为 $\sigma$-环. 函数 $\lambda: \mathscr{S} \to [0, +\infty]$ 称为 $\mathscr{S}$ 上的\alert{外测度}, 若
\begin{enumerate}
\item $\lambda(\emptyset) = 0$;
\item \textbf{单调性}: $E_1 \subset E_2$, $E_i \in \mathscr{S} \implies \lambda E_1 \leqslant \lambda E_2$;
\item \textbf{半可加性}: $E_k \in \mathscr{S}$ 且 $\bigcup_k E_k \in \mathscr{S} \implies \lambda\big(\bigcup_k E_k\big) \leqslant \sum_k \lambda E_k$.
\end{enumerate}
若 $\mathscr{S} = \mathscr{P}(X)$, 称 $\lambda$ 为 $X$ 上的外测度. \alert{外测度没有可列可加性}, 这正是它与测度的差别.
\end{Def}

\pause

\begin{remark}
§2.4 的矩体 L-覆盖外测度 $m^*$ 就是 $\mathscr{P}(\mathbb{R}^n)$ 上的外测度. 当时的覆盖材料是矩体, 现在的问题是: \alert{能否从环 $\mathscr{R}$ 上的测度 $\mu$ 出发, 造出一个外测度?}
\end{remark}
...
\begin{frame}{由环上测度导出外测度}

\begin{Def}[可列覆盖类与导出外测度]
设 $\mathscr{R}$ 为环, $\mu$ 为 $\mathscr{R}$ 上的测度. 记
\[
\mathscr{S}(\mathscr{R}) := \big\{ E :\ E \subset \textstyle\bigcup_{n=1}^\infty A_n,\ A_n \in \mathscr{R} \big\}
\qquad \big(\mathscr{R} \subset \mathscr{R}_\sigma(\mathscr{R}) \subset \mathscr{S}(\mathscr{R})\big),
\]
它是含 $\mathscr{R}$ 的 $\sigma$-环; 若基本集 $X$ 可表为 $\mathscr{R}$ 中可列个集之并, 则 $\mathscr{S}(\mathscr{R}) = \mathscr{P}(X)$. 对 $E \in \mathscr{S}(\mathscr{R})$, 令
\[
\mu^* E := \inf \Big\{ \sum_{n=1}^\infty \mu A_n \ :\ A_n \in \mathscr{R},\ E \subset \bigcup_n A_n \Big\},
\]
则 $\mu^*$ 是 $\mathscr{S}(\mathscr{R})$ 上的外测度, 称为\alert{由 $\mu$ 导出的外测度}, 且 $\mu^*\big|_{\mathscr{R}} = \mu$ (``$\geqslant$'': 半可加性; ``$\leqslant$'': 取 $A_1 = E$).
\end{Def}

\pause

\begin{remark}
$\mu^*$ 外测度三性质的验证与 §2.4 的 $\varepsilon/2^n$ 论证同型. 取 $\mathscr{R}$ = 半开矩体有限并的环, $\mu$ = 体积: $\mu^*$ 恰是勒贝格外测度 $m^*$, 此时 $\mathscr{S}(\mathscr{R}) = \mathscr{P}(\mathbb{R}^n)$.
\end{remark}
...
\begin{frame}{Carath\'{e}odory 扩张定理}

\begin{Def}[$\lambda$-可测]
设 $\lambda$ 为 $\sigma$-环 $\mathscr{S}$ 上的外测度. 称 $E \in \mathscr{S}$ \alert{$\lambda$-可测}, 若对一切 $A \in \mathscr{S}$,
\[
\lambda A = \lambda(A \cap E) + \lambda(A \setminus E)
\qquad \text{(\alert{Carath\'{e}odory 条件}, 试验集限制在 $\mathscr{S}$ 内)}.
\]
$\lambda$-可测集全体记为 $\mathscr{M}$.
\end{Def}

\pause

\begin{thm}[Carath\'{e}odory 扩张定理]
设 $\mu$ 为环 $\mathscr{R}$ 上的测度, $\mu^*$ 为其在 $\mathscr{S}(\mathscr{R})$ 上导出的外测度, $\mathscr{M}$ 为 $\mu^*$-可测集类. 则 $\mathscr{M}$ 是 $\sigma$-环, $\mu^*\big|_{\mathscr{M}}$ 是测度 (可列可加), 且
\[
\mathscr{R} \ \subset\ \mathscr{R}_\sigma(\mathscr{R}) \ \subset\ \mathscr{M} \ \subset\ \mathscr{S}(\mathscr{R}),
\qquad
\mu^*\big|_{\mathscr{R}_\sigma(\mathscr{R})} = \mu :
\]
$\mu$ 被扩张为 $\mathscr{R}_\sigma(\mathscr{R})$ (进而 $\mathscr{M}$) 上的测度.
\end{thm}
...
\begin{frame}{扩张定理的证明要点}

\begin{itemize}
\item \textbf{$\mathscr{M}$ 是 $\sigma$-环、$\mu^*\big|_{\mathscr{M}}$ 可列可加}: 与 §2.4 性质 (2)--(4) 的证明\alert{逐字同型} (对称性、四块分解、互斥分解逐块展开), 只是试验集限制在 $\mathscr{S}(\mathscr{R})$ 内;
\pause
\item \textbf{$\mu^*\big|_{\mathscr{R}} = \mu$}: 见前一页 (半可加性 $+$ 自身覆盖);
\pause
\item \textbf{$\mathscr{R}_\sigma(\mathscr{R}) \subset \mathscr{M}$} (实质性的一步): 由 $\mu^*$ 的定义, $\forall\, \varepsilon > 0$ 可取 $\mathscr{R}$ 中覆盖列 $\{A_n\}$ 使 $\sum_n \mu A_n \leqslant \mu^* A + \varepsilon$ (\alert{任意精度的覆盖逼近}), 用它验证 Carath\'{e}odory 条件 (与 L-覆盖版的论证相同, 细节见教材).
\end{itemize}
...
\begin{frame}{勒贝格测度: ``体积''的扩张}

\begin{thm}[勒贝格测度作为扩张特例]
取 $\mathscr{R}$ 为 $\mathbb{R}^n$ 中\alert{半开矩体有限并}构成的环, $\mu$ = 体积 (它是环上的测度: 可列可加性即 §2.2 开集测度三性质的重述). 扩张定理给出
\[
\mathscr{R} \ \subset\ \mathscr{B}(\mathbb{R}^n) = \sigma(\mathscr{R}) \ \subset\ \mathscr{M} \ \subset\ \mathscr{P}(\mathbb{R}^n),
\]
且 $\mu^*\big|_{\mathscr{B}(\mathbb{R}^n)}$ 就是通常的勒贝格测度 $m$; $\mu^*$-可测集类 (自动完备: 零测集的子集可测) 恰是 \alert{Lebesgue 可测集类}, §2.2--2.4 的理论正是这台机器的产出.
\end{thm}

\pause

\begin{remark}
第三章的可测函数、第四章的勒贝格积分, 全部在 $(\mathbb{R}^n, \mathscr{M}, m)$ 上展开; 一般测度空间 $(X, \mathscr{R}, \mu)$ (如概率测度、L-S 测度) 的理论与此完全平行.
\end{remark}
...
\section[非 Borel 集]{选讲: 正测度集的结构与非 Borel 可测集}
...
\textbf{目标}: 证明 $\mathscr{B}(\mathbb{R}) \subsetneqq \mathscr{M}$, 即存在可测而非 Borel 的集合.
...
\item 一般语言 (扩张定理): 对环 $\mathscr{R}$ 上的测度 $\mu$, $\mathscr{R} \subset \mathscr{R}_\sigma(\mathscr{R}) \subset \mathscr{M} \subset \mathscr{S}(\mathscr{R})$, 测度链条 $\mu \longrightarrow \mu^* \longrightarrow \mu^*\big|_{\mathscr{M}}$; $\mathbb{R}^n$ 情形即上图.
\end{itemize}
...
\pause
\item \textbf{一般测度与扩张定理}: 环上测度 $\mu$ 的连续性定理; 导出外测度 $\mu^* E = \inf\sum_n \mu A_n$; Carath\'{e}odory 扩张定理 ($\mathscr{R} \subset \mathscr{R}_\sigma(\mathscr{R}) \subset \mathscr{M} \subset \mathscr{S}(\mathscr{R})$, $\mu^*$ 扩张 $\mu$); 勒贝格测度 = ``体积''的扩张;
\pause
...
\begin{frame}{展望: 通往第三章}

\begin{itemize}
\item 测度论就绪: $(\mathbb{R}^n, \mathscr{M}, m)$ 已由``体积 $+$ Carath\'{e}odory 机器''建成; 一般测度空间 $(X, \mathscr{R}, \mu)$ (概率测度、L-S 测度) 的理论与之完全平行;
\pause
\item \textbf{第三章 可测函数}: 定义在可测集上、``原像保可测''的函数; Weierstrass 式的逼近 (简单函数逼近、Luzin 定理) 与各种收敛性 (Egorov、Riesz);
\pause
\item \textbf{第四章 勒贝格积分}: 对简单函数按``测度加权''定义积分, 再逼近一般函数; 单调收敛、Fatou、控制收敛与 Fubini 定理都在这台测度机器上运转.
\end{itemize}
