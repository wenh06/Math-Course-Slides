...
\startproof[bg=yellow, fg=red](证明: 只需证 $m^*\mathscr{C}E + m_*E = b - a$)
第一式由 $\mathscr{C}E$ 代 $E$ (注意 $\mathscr{C}(\mathscr{C}E) = E$) 即得; $\mathscr{C}E = \emptyset$ 时显然.

\pause

\textbf{1$^\circ$} 任取 $\varepsilon > 0$. 由 $m_* E = \sup_{F \subset E} mF$, 可取闭集 $F \subset E$ 使得 $mF > m_* E - \varepsilon$. 令 $G = (a, b) \setminus F$, 则 $G$ 为开集, 且 $G \supset \mathscr{C}E$, 从而 $mG \geqslant m^* \mathscr{C}E$; 同时由开集、闭集测度的定义, $mF = b - a - mG$. 于是
\[
m_* E + m^* \mathscr{C}E < mF + \varepsilon + mG = (b - a) + \varepsilon .
\]
由 $\varepsilon$ 任意, $m_* E + m^* \mathscr{C}E \leqslant b - a$.

\end{frame}

%------------------------------------------------

\begin{frame}{对偶公式的证明 (续) 与补集可测}

\startproof[bg=yellow, fg=red](2$^\circ$)
另一方面, 由 $m^* \mathscr{C}E = \inf_{G \supset \mathscr{C}E} mG$, 可取开集 $G \supset \mathscr{C}E$ (不妨 $G \subset (a,b)$, 否则以 $G \cap (a,b)$ 代之) 使得 $mG < m^* \mathscr{C}E + \dfrac{\varepsilon}{2}$. 令 $F = \big[ a + \tfrac{\varepsilon}{4},\ b - \tfrac{\varepsilon}{4} \big] \setminus G$, 则 $F$ 为闭集, 且 $F \subset (a,b) \setminus G \subset (a,b) \setminus \mathscr{C}E = E$. 注意 $(a,b) \setminus F = \big(a, a + \tfrac{\varepsilon}{4}\big) \cup \big(b - \tfrac{\varepsilon}{4},\ b\big) \cup G$, 于是
\begin{align*}
m_* E \;\geqslant\; mF
&= (b - a) - m\big( (a, a + \tfrac{\varepsilon}{4}) \cup (b - \tfrac{\varepsilon}{4},\ b) \cup G \big)\\
&\geqslant (b - a) - \frac{\varepsilon}{2} - mG
\;>\; (b - a) - m^* \mathscr{C}E - \varepsilon ,
\end{align*}
由 $\varepsilon$ 任意, $m^* \mathscr{C}E + m_* E \geqslant b - a$. 结合 1$^\circ$:
\[
m^* \mathscr{C}E + m_* E = b - a .
\]
\qed
