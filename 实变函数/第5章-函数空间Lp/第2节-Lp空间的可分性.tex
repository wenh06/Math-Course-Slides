...
\begin{itemize}
\item 上一节证明了 $L^p$ 空间的\alert{完备性}; $L^p$ 空间还有一个比较重要的性质
  --- \alert{可分性}: 存在\alert{可列}的稠密子空间;
\item 类比: $\mathbb{Q}$ 是 $\mathbb{R}$ 中的\alert{可列稠密子集}
  --- 有理数足够``稠'', 又足够``少'' (可列), 于是 $\mathbb{R}$ 的结构可以通过
  $\mathbb{Q}$ 来把握;
...
\begin{itemize}
\item 路线: 主线 --- \alert{三步逼近引理} (有界可测 $\to$ 简单 $\to$ 有理系数);
  支线 --- Weierstra\ss{} 逼近 (多项式, 超纲补充); 收尾 --- \alert{弱收敛}及收敛关系总结.
\end{itemize}
...
\begin{itemize}
\item \textbf{技巧的核心}: $|h| \leqslant 2M$ 时 $|h|^p \leqslant (2M)^{p-1} |h|$
  --- ``降一次幂'', 而 $\varphi_i$ 的逼近精度恰好带一个 $(2M)^{p-1}$ 分母, 两者相消;
...
\item (a) $\lVert g \rVert_q \geqslant \lVert \varphi_g \rVert
  := \sup_{\lVert f \rVert_p = 1} \Big| \int_E f g ~ \mathrm{d} m \Big|$
  --- Hölder 给出"$\leqslant$"方向; \alert{等号}成立吗?
