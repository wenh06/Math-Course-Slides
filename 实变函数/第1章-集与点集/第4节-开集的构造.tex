...
我们已经学习了开集、闭集的概念 (\textbf{只是描述性的, 非构造性的}), 以及它们的一些性质. 本节我们将给出 $\mathbb{R}$ 中开集的\textbf{构造性}描述, 即任一开集都可以表示为一些开区间的并, 反之任意一些开区间的并也是开集. 这一结果是 $\mathbb{R}$ 的特殊性质 (依赖于 $\mathbb{R}$ 的全序结构); 在高维欧氏空间 $\mathbb{R}^n$ ($n \geqslant 2$) 中, 开集不一定能表示为互不相交开球之并.\par
}
...
设 $F$ 闭. 令 $G_n = \{x : d(x, F) < 1/n\},$ 则各 $G_n$ 是开集 ($d(\cdot, F)$ 连续, 见后文「点到集合的距离」一节), 且
\[
...
\begin{eg}
\begin{itemize}
\item $\mathbb{Q}$ 是 $F_\sigma$ 集 (可列个单点集之并), 但不是 $G_\delta$ 集 (完整证明需要利用 \alert{Baire 纲定理}: 完备度量空间不是可列个无处稠密集之并).
...
\textbf{(2) 无处稠密.} 用反证法: 若 $\mathring{P_0} \neq \emptyset$, 则存在开区间 $(a, b) \subset P_0$, 于是 $(a, b) \subset F_n$ 对每个 $n$ 成立. 取 $n$ 充分大使 $3^{-n} < b - a$; 由于 $(a, b)$ 是区间, 它必整个落在 $F_n$ 的某个构成区间内, 但 $F_n$ 的每个构成区间长度为 $3^{-n} < b - a$, 矛盾. 故 $\mathring{P_0} = \emptyset$.
...
\]
\pause
由闭区间套定理, 存在唯一的 $\xi \in \bigcap_{n=1}^\infty E_n.$ 每个 $E_n = F_{n k_n}$ 的两个端点都是被挖去开区间的端点 (若某端点为 $0$ 或 $1$, 则它本身属于 $P_0$), 因而属于 $P_0,$ 且与 $\xi$ 的距离不超过 $3^{-n} \to 0,$ 故 $\xi$ 是 $P_0$ 的聚点; 又 $P_0$ 是闭集, 故 $\xi \in P_0.$
\pause
...
\begin{frame}{补充: Baire 纲定理证明}

\textbf{关键断言:} 非空完备度量空间 $(X, d)$ 中, 可列个稠密开集之交非空.

\pause

\textbf{断言的证明.} 设 $\{G_k\}$ 为稠密开集列. 由 $G_1$ 稠密、$X \neq \emptyset$ 知 $G_1 \neq \emptyset$, 取闭球 $\overline{B}(x_1, r_1) \subset G_1$, $0 < r_1 < 1$. 归纳地, 已得 $\overline{B}(x_{k-1}, r_{k-1})$ 后, 由 $G_k$ 稠密, 非空开集 $G_k \cap B(x_{k-1}, r_{k-1})$ 内可取闭球
\[
  \overline{B}(x_k, r_k) \subset G_k \cap B(x_{k-1}, r_{k-1}), \qquad 0 < r_k < \tfrac{1}{k},
\]
得嵌套闭球列, 半径 $r_k \to 0$. 当 $j \geqslant k$ 时 $x_j \in \overline{B}(x_k, r_k)$, 故 $d(x_j, x_k) \leqslant r_k$, $\{x_k\}$ 是 Cauchy 列; 由完备性 $x_k \to \xi \in X$. 固定 $k$, 令 $j \to \infty$ 并用 $\overline{B}(x_k, r_k)$ 的闭性, 得 $\xi \in \overline{B}(x_k, r_k) \subset G_k$ 对一切 $k$ 成立, 即 $\bigcap_k G_k \neq \emptyset$.

\pause

\textbf{定理的证明.} 反设 $X = \bigcup_{n=1}^{\infty} E_n$, 各 $E_n$ 无处稠密. 以 $\overline{E_n}$ 代替 $E_n$ (仍无处稠密), 不妨设诸 $E_n$ 闭; 则 $G_n := X \setminus E_n$ 是稠密开集, 且 $\bigcap_n G_n = X \setminus \bigcup_n E_n = \emptyset$, 与断言矛盾. \qed

\pause

\begin{remark}
$\mathbb{R}$ 情形闭球即闭区间, 上述构造就是闭区间套. 它应用于 $\mathbb{Q}$ 非 $G_\delta$ 集、无理数集非 $F_\sigma$ 集的证明, 见前文「Borel 集类」帧.
\end{remark}
