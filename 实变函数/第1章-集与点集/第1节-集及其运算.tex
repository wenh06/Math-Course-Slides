...
\begin{remark}
集合的包含关系 $\subset$ 是一个\alert{偏序关系} (满足自反、反对称、传递), 但一般不是全序关系: 例如 $\{1, 2\}$ 与 $\{1, 3\}$ 互不包含.
\end{remark}
...
\item \alert{描述法}: $\{x \in X : P(x)\},$ $P$ 是 $x$ 的某种性质或条件. 例如 $\{x \in \mathbb{R} : x^2 < 2\}$; 又如 $\{f : f \text{ 是 } \mathbb{R} \text{ 上的连续函数},\ f(1) = 0\}$ (以函数为元素的集合).
\end{itemize}
\end{block}
...
\iff x \in \bigcap_{\alpha \in I} \mathscr{C} A_\alpha.
\end{align*}
\qed
...
\medskip
$\subset$：若 $x \in \varliminf_{n\to\infty} E_n$，则从某个 $N$ 起 $x \in E_n$. 设 $x = p/q$（既约, $q > 0$）: 对一切 $n \geqslant N$ 有 $q \mid np$, 由 $(p, q) = 1$ 得 $q \mid n$; 取相邻两个 $n$ 得 $q = 1$, 即 $x \in \mathbb{Z}$.
...
\begin{block}{教材第一章 §1 习题}
\begin{itemize}
\item \textbf{3.} 设给出集 $E$ 与任一集族 $\{A_\alpha\}_{\alpha \in I}$, 问关系式
$E \cup \bigcap_{\alpha \in I} A_\alpha = \bigcap_{\alpha \in I} (E \cup A_\alpha)$
是否恒成立? (即正文中的推论.)
\item \textbf{5.} 定义集 $A, B$ 的对称差为 $A \triangle B = (A \setminus B) \cup (B \setminus A)$. 试证对任意集 $A, B, C$:
